\chapter{Fonctions des plusieurs variables réelles ; limites et continuité}

\begin{definition}[limite de fonction] \label{def: limite de fonction}
Soit $ f : E \to \R$ et $\bm x_0 \in \Rn$ un point d'accumulation de $E$. On dit que $\lim_{\bm x \to \bm x_0} f(\bm x)$ existe et est égale à $l \in \R$ si,
	\[
...
\begin{theorem}[Caractérisation de la limite par les suites] \label{thm : Caractérisation de la limite par les suites}
	Soit $f : E \to \R$ et $\bm x_0 \in \Rn$ un point d'accumulation de $E$. Alors $f$ admet pour limite $l$ lorsque $\bm x$ tend vers $\bm x_0$ si et seulement si,
	\[
...
\begin{theorem}[Théorème des gendarmes] \label{thm: théorème des gendarmes}
	Soit $f,g,h : E \to \R$ et $\bm x_0 \in \Rn$ un point d'accumulation de $E$. Si $\lim_{\bm x \to \bm x_0} h(\bm x) = \lim_{\bm x \to \bm x_0} g(\bm x) = l$ et, s'il existe $\delta > 0$ tel que,
	\[
...
\begin{proposition}
	Dans la pratique, on utilise souvent une fonctions $g$ qui dépend uniquement de la distance $\norme{\bm x - \bm x_0}$, on pose donc
	\[
...
\begin{theorem}[Critère de Cauchy] \label{thm: Critère de Cauchy}
Soit $f : E \to \R$ et $\bm x_0 \in \R$ un point d'accumulation de $E$. Alors $\lim_{\bm x \to \bm x_0} f(\bm x)$ existe si et seulement si
	\[
...
\section{Limite de fonction à valeurs dans $\Rm$}

\begin{definition}Soit $\bm f: E \to \Rm$ et $\bm x_0$ un point d'accumulation de $E$. On dit que $\lim_{\bm x \to \bm x_0} \bm f(\bm x) = \bm l \in \Rm$ si et seulement si
	\[
...
\begin{definition}[Caractérisation de la continuité] \label{def: Caractérisation de la continuité}
On peut définit la \emph{continuité} de plusieurs manière équivalentes ; Soit $f : E \to \R$ et $\bm x_0 \in \Rn$ un point d'accumulation de $E$. On dit que $f$ est continue en $\bm x_0$ si et seulement si,
	\begin{itemize}
		\item \[
...
\begin{theorem}[Composition de fonctions continues] \label{thm: Composition de fonctions continues}
	Soit $\bm f : E \subset \Rn \to \Rm$ continue en $\bm x_0 \in E$ et $\bm g : A \to \Rn$ continue en $\bm y_0$ et telle que $\bm x_0 = \bm g(\bm y_0)$. Alors $\bm h = \bm f \circ \bm g$ est continue en $\bm y_0$.
\end{theorem}
...
\begin{definition}[Uniforme continuité] \label{def: Uniforme continuité}
	On dit que $f : E \to \R$ est \emph{uniformément continue} sur $E$ si et seulement si,
	\[
...
\begin{proposition} 
	Soit $f : E \to \R$ une fonction uniformément continue, alors $f$ est continue.
\end{proposition}
...
\section{Prolongement de fonctions par continuité}
\begin{theorem}[Prolongement par continuité]Soient $E \subset \Rn$ non vide tel que $\overline E \neq E$ et $ \bm f : E \to \Rm$ continue. Supposons que pour tout $\bm x \in \overline E \backslash E$,
	\[
	\exists \bm l \in \Rm, \quad \lim_{\bm y \to \bm x} \bm f(\bm y) = \bm l
	\]
	On pose donc
	\[
	\begin{array}{cccc}
		\bm{\tilde f}  : & \overline E & \to & \Rm \\
...
\begin{theorem}[Prolongement par uniforme continuité] \label{thm: Prolongement par unfirme continuité}
	Soit $E \subset \Rn$ non-vide et $\bm f: E \to \Rm$ une fonction uniformément continue sur $E$. Alors $\bm f$ peut être prolonger par continuité sur $\overline E$ et son prolongement continu $\bm{\tilde f} : \overline E \to \Rm$ est aussi uniformément continue.
\end{theorem}
...
\prf{Par hypothèse d'uniforme continuité,
	...
	\[
	\exists \bm l \in \Rm, \quad  \lim_{k \to \infty} \bm f(\bm x^k) = \bm l
	\]
...
\section{Fonction continues sur un compact}
\lecture{10 mars}
\begin{theorem}
	Soit $E$ un compact non vide et  $f : E \to \R$	 une fonction continue. Alors
	\begin{itemize}
		\item $f$ est bornée
...
\begin{theorem}[Théorème de Cantor-Heine généralisé] \label{thm: théorème de Cantor-Heine généralisé}
	Soient $E \subset \Rn$ un ensemble quelconque, $K \subset E$ un sous-ensemble non-vide et compact, et $\bm f : E \to \Rm$ une fonction continue. Alors
	\[
